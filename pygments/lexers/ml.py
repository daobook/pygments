...
class SMLLexer(RegexLexer):
    ...
    def long_id_callback(self, match):
        token = Error if match.group(1) in self.alphanumid_reserved else Name.Namespace
...
class OpaLexer(RegexLexer):
    ...
    tokens = {
        ...
        'root': [
            ...
            (f'@{ident_re}\\b', Name.Builtin.Pseudo),
            (r'-?.[\d]+([eE][+\-]?\d+)', Number.Float),
            (r'-?\d+.\d*([eE][+\-]?\d+)', Number.Float),
            (r'-?\d+[eE][+\-]?\d+', Number.Float),
        ...
        'type-1': [
            ...
            (f'{ident_re}\\(', Keyword.Type, ('#pop', 'type-tuple')),
