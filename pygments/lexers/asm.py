...
class HsailLexer(RegexLexer):
    ...
    register = f'(\\$(c|s|d|q){register_number})\\b'
    # Qualifiers
    alignQual = r'(align\(\d+\))'
    ...
    tokens = {
        'root': [
            ...
            include('keyword'),
            (f'&{identifier}', Name.Variable.Global),
            (f'%{identifier}', Name.Variable),
            (hexfloat, Number.Hex),
        ...
        'keyword': [
            (f'kernarg{datatypeMod}', Keyword.Type),
            ...
            (roundingMod, Keyword),
            (datatypeMod, Keyword),
            (f'_({alignQual}|{widthQual})', Keyword),
            (r'_kernarg', Keyword),
...
class LlvmLexer(RegexLexer):
    ...
    #: optional Comment or Whitespace
    string = r'"[^"]*?"'
    identifier = f'([-a-zA-Z$._][\\w\\-$.]*|{string})'
    block_label = f'({identifier}|(\\d+))'

    tokens = {
        'root': [
            include('whitespace'),
            (f'{block_label}\\s*:', Name.Label),
            include('keyword'),
            (f'%{identifier}', Name.Variable),
            (f'@{identifier}', Name.Variable.Global),
